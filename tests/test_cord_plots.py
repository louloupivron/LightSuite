...
def test_select_axial_slices_spans_full_volume() -> None:
    volume = np.zeros((40, 20, 921), dtype=np.uint8)
    annotation = np.zeros((40, 20, 921), dtype=np.uint16)
    volume[15:25, 8:12, 300:622] = 200
    annotation[16:24, 7:13, 320:600] = 5

    slices = _select_axial_slices(volume, annotation, n_show=12, bg_threshold=2)

    assert slices.shape == (12,)
    assert int(slices[0]) < 80
    assert int(slices[-1]) > 840
