...
def export_registered_cord_volumes(
    ...
    *,
    ...
    export_spaces = spaces if spaces is not None else config.export.spaces
    spaces_set = {str(s).strip().lower() for s in export_spaces}
    if not spaces_set:
        msg = "At least one export space (atlas or sample) must be selected."
