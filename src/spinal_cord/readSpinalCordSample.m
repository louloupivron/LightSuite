function [finvol, opts] = readSpinalCordSample(dp, sampleres, tifftype)
%READSPINALCORDSAMPLE Load a spinal cord lightsheet TIFF stack.
%   [finvol, opts] = readSpinalCordSample(dp, sampleres)
%   [finvol, opts] = readSpinalCordSample(dp, sampleres, tifftype)
%
...
switch layout
    case 'planeperfile'
        ...
        finvol = localLoadPlanePerFileStack(tfiles);
    case 'channelperfile'
        if numel(tfiles) == 1
...
% Many single-page TIFFs in one folder => one slice per file.
...
%--------------------------------------------------------------------------
function finvol = localLoadPlanePerFileStack(tfiles)
